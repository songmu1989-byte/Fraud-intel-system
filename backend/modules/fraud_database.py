"""
涉诈信息库 — 基于 JSON 文件的轻量级持久化存储 (完整修复版)
修复了 url 字段为 None 时导致的 search_records 崩溃问题
"""
import json
import os
import uuid
...
_SEED_DATA = [
    ...
    },
    {
        ...
        "company": "新远教育科技", "url": "xinyuan-edu.com", "input_type": "recruitment_text",
...
def add_record(record: Dict) -> str:
    ...
    save_db(db)
    logger.info(f"[DB] 新增记录 #{record['id']}")
    return record["id"]

def search_records(query: str = "", fraud_type: str = "", risk_level: str = "") -> List[Dict]:
    """多条件查询 - 修复了 None 导致的崩溃问题"""
    records = load_db()["records"]
    if query:
        q = query.lower()
        # 修复逻辑：使用 str(x or "") 确保即使字段是 None 也能安全调用 .lower()
        records = [
            r for r in records
            if q in str(r.get("company") or "").lower()
            or q in str(r.get("url") or "").lower()
            or q in str(r.get("fraud_type") or "").lower()
            or any(q in str(e or "").lower() for e in r.get("evidence", []))
        ]
    if fraud_type and fraud_type != "全部":
...
def get_stats() -> Dict:
    ...
    for r in records:
        ft = str(r.get("fraud_type") or "未分类")
        type_counts[ft] = type_counts.get(ft, 0) + 1
        rl = r.get("risk_level", "中")
        if rl in risk_counts:
            risk_counts[rl] += 1

    total_complaints = sum(int(r.get("complaint_count") or 0) for r in records)
    return {
        "total": len(records),
...
def get_related_records(company: str = "", fraud_type: str = "") -> List[Dict]:
    """关联查询：还原了 Claude 原始提供的此功能"""
    records = load_db()["records"]
    related = []
    for r in records:
        if company and company in str(r.get("company") or ""):
            related.append(r)
